...
export class App {
  ...
  protected readonly dark = signal(this.load('clear_dark', false));

  // ─── Main tasks ─────────────────────────────────────────
  protected readonly tasks = signal<Task[]>(this.load('clear_tasks', []));
  ...
  // ─── Secondary tasks ───────────────────────────────────
  protected readonly secondaryTasks = signal<SecondaryTask[]>(this.load('clear_secondary', []));
  ...
  protected readonly secondaryTitle = signal(this.load('clear_sec_title', 'secondary'));
  protected readonly editingSecondaryTitle = signal(false);
  protected readonly secondaryVisible = signal(this.load('clear_sec_visible', true));

  constructor() {
    effect(() => this.save('clear_dark', this.dark()));
    effect(() => this.save('clear_tasks', this.tasks()));
    effect(() => this.save('clear_secondary', this.secondaryTasks()));
    effect(() => this.save('clear_sec_title', this.secondaryTitle()));
    effect(() => this.save('clear_sec_visible', this.secondaryVisible()));
  }
  ...
  protected readonly canAddSecondary = computed(() => this.secondaryCount() < 5);
